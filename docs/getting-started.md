...
`macos-ui` 会解析到 `yanxulang/yanxu-macos-ui`。需要固定版本时可追加版本约束，例如 `--version '^0.3'`。
...
处理器收到 `麦金塔事件`，其中 `名称` 是稳定事件名，`载荷` 至少包含 `source`。文本框、密码框、多行文本和开关用 `.设事件（「字段.changed」）` 注册变更事件，载荷还包含当前 `value`。
