...
当前 `0.1.x` 包清单保持纯言序形态，不声明原生制品，避免安装阶段要求用户拥有 Swift 工具链或预构建动态库。`界面.运行（应用）` 通过 `标准:进程` 启动 runner，由 runner 读取应用 JSON 并打开窗口。等发布按平台区分、带 SHA-256 校验的动态库后，再按言序原生扩展协议加入清单。
